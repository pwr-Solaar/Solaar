...
def create(receiver):
    ...
    assistant = _create_assistant(receiver, _prepare, _finish, title, text)
    return assistant


def _prepare(assistant, page, receiver):
    index = assistant.get_current_page()
    print("PREPARE", index, receiver.name, page)
    print("PREPARE", page.get_children())
    ...
    if index == 0:
        if receiver.receiver_kind == "bolt":
            ...
            spinner = page.get_children()[-1]
            print("PAGE", spinner, page.get_children())
            spinner.start()
            GLib.timeout_add(_STATUS_CHECK, _check_lock_state, assistant, receiver)
            assistant.set_page_complete(page, True)
        else:
...
def _check_lock_state(assistant, receiver, count=2):
    ...
    if receiver.pairing.error:
        ...
        return False
    elif receiver.pairing.device_passkey:
        _show_passcode(assistant, receiver, receiver.pairing.device_passkey)
        return True
    elif receiver.pairing.device_address and receiver.pairing.device_name:
        address = receiver.pairing.device_address
        entropy = 10 if receiver.pairing.device_kind == _hidpp10_constants.DEVICE_KIND.keyboard else 20
        if receiver.pair_device(address=address, authentication=receiver.pairing.device_authentication, entropy=entropy):
            return True
        else:
            _pairing_failed(assistant, receiver, "failed to open pairing lock")
            return False
...
def _pairing_failed(assistant, receiver, error):
    ...
    receiver.reset_pairing()  # clear out any information from this pairing
    assistant.commit()
...
def _create_assistant(receiver, prepare, finish, title, text):
    ...
    assistant.connect("prepare", prepare, receiver)
