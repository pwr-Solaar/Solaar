...
def _parse_config(loaded_config, config_path):
    ...
    try:
        ...
        discard_capabilities = loaded_version != current_version
        if discard_capabilities:
            _log.warning(
        ...
        for device in loaded_config[1:]:
            ...
            parsed_config.append(_device_entry_from_config_dict(device, discard_capabilities))
    except Exception as e:
        _log.warning('Exception processing config file \'%s\', ignoring contents: %s', config_path, e)
    return parsed_config


def _device_entry_from_config_dict(data, discard_capabilities):
