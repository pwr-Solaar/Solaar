...
def _parse_config(loaded_config, config_path):
    ...
    try:
        ...
        if loaded_version != current_version:
            _log.warning('config file \'%s\' was generated by another version of solaar '
                         '(config: %s, current: %s). ignoring contents', config_path, loaded_version, current_version)
            return parsed_config

        for device in loaded_config[1:]:
            assert isinstance(device, dict)
            parsed_config.append(_device_entry_from_config_dict(device))
    except Exception as e:
        _log.warning('Exception processing config file \'%s\', ignoring contents: %s', config_path, e)
    return parsed_config


def _device_entry_from_config_dict(data):
